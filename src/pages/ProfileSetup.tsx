...
import { Certification, Achievement, ResearchPaper } from '@/types';
...
const ProfileSetup = () => {
  ...
  const [certifications, setCertifications] = useState<Certification[]>([{ id: uuidv4(), name: '', issuer: '', date: '' }]);
  const [achievements, setAchievements] = useState<Achievement[]>([{ id: uuidv4(), title: '', description: '' }]);
  const [researchPapers, setResearchPapers] = useState<ResearchPaper[]>([{ id: uuidv4(), title: '', url: '' }]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (user) {
        ...
        if (error) throw new Error('Error saving profile');
      }
    ...
    <div className="min-h-screen bg-skill-gray py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-center space-x-2 mb-8">
          ...
          <h1 className="text-3xl font-bold text-skill-darkGray">Complete Your Profile</h1>
        </div>
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-6">
              ...
              {/* Hackathon Details */}
              <div className="border-t pt-6">
                <h3 className="text-lg font-medium mb-4">Hackathon Experience</h3>
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setHackathonParticipation(parseInt(e.target.value))}
                      required
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="hackathonWins">Hackathon Wins</Label>
                    <Input
                      id="hackathonWins"
                      type="number"
                      min="0"
                      value={hackathonWins}
                      onChange={(e) => setHackathonWins(parseInt(e.target.value))}
                      required
                    />
                  </div>
                </div>
